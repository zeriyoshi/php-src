--TEST--
Test function: rng_range() function in 64bit range.
--SKIPIF--
<?php
if (PHP_INT_SIZE < 8) {
...
foreach (include(__DIR__ . DIRECTORY_SEPARATOR . '_rng_classes.inc') as $class => $is_seed) {
    ...
    foreach (range(0, 999) as $i) {
        $next = rng_range($rng, 4294967296, PHP_INT_MAX);
...
die('OK, rng_range() works correctly on 64bit.');
?>
--EXPECT--
OK, rng_range() works correctly on 64bit.
